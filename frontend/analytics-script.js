...
function changeMode(selected) {
    ...
    document.querySelector(`[onclick="changeMode('${selected}')"]`).classList.add("active");
    loadCharts();
}
...
async function loadCharts() {
    try {
        ...
        if (topPostElement) {
            topPostElement.innerText = 
                `${data.topPost.text.substring(0, 100)}${data.topPost.text.length > 100 ? '...' : ''} — ${data.topPost.likes} Likes`;
        }
        
        if (trendingTagElement) {
            trendingTagElement.innerText = 
                `${data.trendingHashtag.tag} (${data.trendingHashtag.count} ${data.trendingHashtag.count === 1 ? 'post' : 'posts'})`;
        }
        ...
        if (trendingTagElement) trendingTagElement.innerText = "Check console for errors";
