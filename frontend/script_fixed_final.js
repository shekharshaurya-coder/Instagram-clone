...
async function init() {
  try {
    ...
    // Ensure feed view is active on load
    showFeed({ target: document.querySelector(".toggle-btn:first-child") });
  } catch (error) {
...
async function loadFeed() {
  try {
    ...
    feedContainer.innerHTML =
      '<div style="padding:20px;text-align:center;color:#8b8d91;">Loading feed...</div>';

    const posts = await fetchAPI("/api/posts/feed");

    if (!posts || posts.length === 0) {
      feedContainer.innerHTML =
        '<div style="padding:40px;text-align:center;color:#8b8d91;">No posts yet. Start following people to see their posts!</div>';
      return;
    }
    ...
    feedContainer.innerHTML = posts
      .map((post) => {
        ...
        const avatar = post.author?.avatarUrl
          ? `<img src="${post.author.avatarUrl}" style="width:100%;height:100%;border-radius:50%;object-fit:cover;">`
          : "👤";

        return `
                <div class="post-card" data-post-id="${
                  post.id
                }" style="background:#242526;border-radius:12px;padding:20px;margin-bottom:16px;">
                    <div class="post-header" style="display:flex;gap:12px;margin-bottom:16px;">
                        <div class="post-avatar" style="width:48px;height:48px;border-radius:50%;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);display:flex;align-items:center;justify-content:center;font-size:24px;overflow:hidden;flex-shrink:0;">
                            ${avatar}
                        </div>
                        <div style="flex:1;">
                            <div class="post-name" style="font-weight:600;color:#e4e6eb;font-size:15px;">${
                              post.author?.displayName ||
                              post.author?.username ||
                              "Unknown"
                            }</div>
                            <div class="post-username" style="font-size:13px;color:#8b8d91;">@${
                              post.author?.username || "unknown"
                            } · ${timeAgo}</div>
                        </div>
                    </div>
                    <div class="post-content" style="color:#e4e6eb;font-size:15px;line-height:1.5;margin-bottom:12px;white-space:pre-wrap;">${
                      post.content
                    }</div>
                    ${
                      post.mediaUrl
                        ? `<img src="${post.mediaUrl}" class="post-media" style="width:100%;border-radius:12px;margin-bottom:12px;" alt="Post media">`
                        : ""
                    }
                    <div class="post-actions" style="display:flex;gap:20px;padding-top:12px;border-top:1px solid #3a3b3c;">
                        <button class="action-btn ${
                          post.isLiked ? "liked" : ""
                        }" onclick="toggleLike('${
          post.id
        }')" style="background:none;border:none;color:#8b8d91;cursor:pointer;display:flex;align-items:center;gap:8px;padding:8px 12px;border-radius:8px;transition:0.2s;font-size:14px;">
                            ${post.isLiked ? "❤️" : "🤍"} <span id="likes-${
          post.id
        }">${post.likesCount || 0}</span>
                        </button>
                        <button class="action-btn" onclick="showCommentModal('${
                          post.id
                        }')" style="background:none;border:none;color:#8b8d91;cursor:pointer;display:flex;align-items:center;gap:8px;padding:8px 12px;border-radius:8px;transition:0.2s;font-size:14px;">💬 <span id="comments-${
          post.id
        }">${post.commentsCount || 0}</span></button>
                        <button class="action-btn" style="background:none;border:none;color:#8b8d91;cursor:pointer;display:flex;align-items:center;gap:8px;padding:8px 12px;border-radius:8px;transition:0.2s;font-size:14px;">🔄</button>
                        <button class="action-btn" style="background:none;border:none;color:#8b8d91;cursor:pointer;display:flex;align-items:center;gap:8px;padding:8px 12px;border-radius:8px;transition:0.2s;font-size:14px;">📤</button>
                    </div>
                </div>
            `;
    ...
    document.getElementById("feed-posts").innerHTML =
      '<div style="padding:20px;text-align:center;color:#ff7979;">Failed to load feed. Please refresh the page.</div>';
  }
}
...
function initSocket() {
  ...
  socket.on("new_message", (message) => {
    ...
    // Create server-side notification (non-blocking)
    (async () => {
      try {
        // Only create notification if not currently viewing this conversation
    ...
    container.innerHTML = conversations
      .map((conv) => {
        ...
        return `
          ...
          conv.conversationId
        }')" style="padding:15px;border-bottom:1px solid #2f3336;cursor:pointer;transition:0.2s;">
                    <div style="display:flex;align-items:center;gap:12px;">
                        <div style="position:relative;">
                            <div style="width:50px;height:50px;border-radius:50%;background:linear-gradient(135deg,#667eea 0%,#764ba2 100%);display:flex;align-items:center;justify-content:center;font-size:24px;overflow:hidden;">
                                ${
                                  conv.otherUser.avatarUrl
                                    ? `<img src="${conv.otherUser.avatarUrl}" style="width:100%;height:100%;object-fit:cover;">`
                                    : "👤"
                                }
                            </div>
                            ${
                        ...
                        <div style="flex:1;">
                            <div style="display:flex;justify-content:space-between;align-items:center;">
                                ...
                                <div style="font-size:12px;color:#8b8d91;">${formatMessageTime(
                                  conv.lastMessage.createdAt
                                )}</div>
                            </div>
                            <div style="font-size:14px;color:${
                              conv.unreadCount > 0 ? "#e4e6eb" : "#8b8d91"
                            };${
          conv.unreadCount > 0 ? "font-weight:600;" : ""
        }">${lastMessagePreview}</div>
                        </div>
                        ${
                          conv.unreadCount > 0
                            ? `<div style="background:#667eea;color:white;padding:4px 8px;border-radius:12px;font-size:12px;font-weight:600;">${conv.unreadCount}</div>`
                            : ""
                        }
                    </div>
                </div>
    ...
    try {
      showInAppToast(
        ...
        () => {
          ...
          showMessages({
            target: document.querySelector(".toggle-btn:nth-child(2)"),
          });
          const convId =
            message.conversationId ||
            [currentUser?.id, message.sender.id].sort().join("_");
          openConversation(
            message.sender.id,
      ...
      console.warn("Could not show in-app notification", e);
    }
  }
}
...
function showMessages(event) {
  // show messages view and hide feed/notifications views
  ...
  const globalInput = document.getElementById("globalChatInput");
  if (globalInput) globalInput.style.display = "flex";

  // Show conversations list
  const convList = document.getElementById("conversationsList");
  if (convList) convList.style.display = "block";

  // Hide search results and chat window initially
...
function showFeed(event) {
  ...
  // hide message UI elements
  const globalInput = document.getElementById("globalChatInput");
  if (globalInput) globalInput.style.display = "none";
  const chatWindow = document.getElementById("chatWindow");
  if (chatWindow) chatWindow.style.display = "none";
      ...
      .map((n) => {
        ...
        switch (n.verb) {
          ...
          case "comment":
            ...
            break;
          case "system":
            verbText = n.message || "sent you a notification";
            break;
          default:
            verbText = n.verb;
        }
...
function switchToHome(event) {
  ...
  document
    .querySelectorAll(".toggle-btn")
    .forEach((el) => el.classList.remove("active"));
  const feedBtn = document.querySelector(".toggle-btn:first-child");
...
function createCommentModal() {
  ...
  modal.innerHTML = `
        <div class="modal-content" style="max-width: 500px;">
            <div class="modal-header" style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 20px; border-bottom: 1px solid #3a3b3c; padding-bottom: 16px;">
                <h3 style="margin: 0; color: #fff; font-size: 20px;">Add a Comment</h3>
                <button onclick="closeCommentModal()" style="background: none; border: none; color: #8b8d91; font-size: 24px; cursor: pointer; padding: 0; width: 32px; height: 32px; display: flex; align-items: center; justify-content: center;">×</button>
            </div>
            
            <textarea id="commentInput" placeholder="What's your comment?" style="width: 100%; min-height: 120px; padding: 12px; background: #262626; color: #fff; border: 1px solid #3a3b3c; border-radius: 8px; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, Oxygen, Ubuntu, Cantarell, sans-serif; font-size: 14px; resize: vertical; box-sizing: border-box; margin-bottom: 16px;"></textarea>
            
            <div style="display: flex; gap: 12px; justify-content: flex-end;">
                <button onclick="closeCommentModal()" style="padding: 10px 24px; background: #262626; color: #fff; border: 1px solid #3a3b3c; border-radius: 24px; cursor: pointer; font-weight: 600; transition: 0.2s;">Cancel</button>
                <button onclick="submitComment()" style="padding: 10px 24px; background: var(--primary, #1f77f5); color: #fff; border: none; border-radius: 24px; cursor: pointer; font-weight: 600; transition: 0.2s;">Comment</button>
            </div>
        </div>
...
async function submitComment() {
  ...
  try {
    ...
    if (response) {
      ...
      const countSpan = document.getElementById(`comments-${postId}`);
...
async function loadPostComments(postId) {
  try {
    ...
    console.log(`Fetching comments from: ${API_URL}${endpoint}`);
...
function displayComments(postId, comments) {
  ...
  if (!commentsContainer) {
    ...
    commentsContainer.style.cssText =
      "margin-top: 16px; padding-top: 16px; border-top: 1px solid #3a3b3c;";
  ...
  commentsContainer.innerHTML = comments
    .map(
      (comment) => `
        <div class="comment" style="margin-bottom: 12px; padding: 12px; background: #262626; border-radius: 8px; display: flex; gap: 12px;">
            <div style="flex: 1;">
                <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 4px;">
                    ...
                    ${
                      comment.author?.id === currentUser?.id
                        ? `<button onclick="deleteComment('${comment._id}', '${postId}')" style="background: none; border: none; color: #8b8d91; cursor: pointer; font-size: 12px; padding: 4px 8px;">Delete</button>`
                        : ""
                    }
                </div>
                <p style="margin: 0; color: #8b8d91; font-size: 13px;">${
                  comment.text
                }</p>
                <span style="color: #65676b; font-size: 12px;">${new Date(
                  comment.createdAt
