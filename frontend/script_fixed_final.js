...
async function loadFeed() {
    try {
        ...
        feedContainer.innerHTML = '<div style="padding:20px;text-align:center;color:#8b8d91;">Loading feed...</div>';
        
        const posts = await fetchAPI('/api/posts/feed');
        
        if (!posts || posts.length === 0) {
            feedContainer.innerHTML = '<div style="padding:40px;text-align:center;color:#8b8d91;">No posts yet. Start following people to see their posts!</div>';
            return;
        }
        ...
        feedContainer.innerHTML = posts.map(post => {
            ...
            return `
                <div class="post-card" style="background:#242526;border-radius:12px;padding:20px;margin-bottom:16px;">
                    <div class="post-header" style="display:flex;gap:12px;margin-bottom:16px;">
                        ...
                        <div style="flex:1;">
                            <div class="post-name" style="font-weight:600;color:#e4e6eb;font-size:15px;">${post.author?.displayName || post.author?.username || 'Unknown'}</div>
                            <div class="post-username" style="font-size:13px;color:#8b8d91;">@${post.author?.username || 'unknown'} · ${timeAgo}</div>
                        </div>
                    </div>
                    <div class="post-content" style="color:#e4e6eb;font-size:15px;line-height:1.5;margin-bottom:12px;white-space:pre-wrap;">${post.content}</div>
                    ${post.mediaUrl ? `<img src="${post.mediaUrl}" class="post-media" style="width:100%;border-radius:12px;margin-bottom:12px;" alt="Post media">` : ''}
                    <div class="post-actions" style="display:flex;gap:20px;padding-top:12px;border-top:1px solid #3a3b3c;">
                        <button class="action-btn ${post.isLiked ? 'liked' : ''}" onclick="toggleLike('${post.id}')" style="background:none;border:none;color:#8b8d91;cursor:pointer;display:flex;align-items:center;gap:8px;padding:8px 12px;border-radius:8px;transition:0.2s;font-size:14px;">
                            ${post.isLiked ? '❤️' : '🤍'} <span id="likes-${post.id}">${post.likesCount || 0}</span>
                        </button>
                        <button class="action-btn" style="background:none;border:none;color:#8b8d91;cursor:pointer;display:flex;align-items:center;gap:8px;padding:8px 12px;border-radius:8px;transition:0.2s;font-size:14px;">💬 ${post.commentsCount || 0}</button>
        ...
        document.getElementById('feed-posts').innerHTML = '<div style="padding:20px;text-align:center;color:#ff7979;">Failed to load feed. Please refresh the page.</div>';
    }
}
                ...
                if (currentConversation !== message.conversationId) {
                    await fetchAPI('/api/notifications', {
                        ...
                        body: JSON.stringify({
                            ...
                            targetType: 'Message',
                            targetId: message.id,
                            message: `New message from ${message.sender?.displayName || message.sender?.username}`,
                            read: false
                        })
                    });
...
async function loadConversations() {
    try {
        ...
        if (!conversations) { container.innerHTML = '<div style="padding:20px;text-align:center;color:#8b8d91;'>No conversations yet</div>'; return; }
        if (conversations.length === 0) { container.innerHTML = '<div style="padding:20px;text-align:center;color:#8b8d91;'>No conversations yet</div>'; return; }
        ...
        try {
            showInAppToast(
                ...
                () => {
                    ...
                    showMessages({ target: document.querySelector('.toggle-btn:nth-child(2)') });
                    const convId = message.conversationId || [currentUser?.id, message.sender.id].sort().join('_');
    ...
    try {
        ...
        container.innerHTML = notifications.map(n => {
            ...
            switch (n.verb) {
                ...
                case 'system': verbText = n.message || 'sent you a notification'; break;
    ...
    try {
        ...
        try {
            showInAppToast(title, body, icon, () => { 
                const convId = message.conversationId || ([currentUser && currentUser.id, message.sender.id].filter(Boolean).sort().join('_')); 
                openConversation(message.sender.id, message.sender.username, message.sender.displayName, message.sender.avatarUrl || '', convId); 
                window.focus(); 
            }, 'Reply');
        } catch (e) {
